...
if __name__ == '__main__':
	...
	if args.viz:
		...
		viz_ = Visualizations(device, type='data')
		...
		if load_plotting:
			for i in range(10):
				with torch.no_grad():
					...
					print("plotting....")
					if isinstance(model, LatentODE) and (
							args.dataset == "periodic" or args.dataset == "warp"):  # and not args.classic_rnn and not args.ode_rnn:
						plot_id = 10000 + i
		...
		if itr % (n_iters_to_viz * num_batches) == 0:
			...
			if args.viz:
				with torch.no_grad():
					...
					print("plotting....")
					if isinstance(model, LatentODE) and (args.dataset == "periodic" or args.dataset == "warp"): #and not args.classic_rnn and not args.ode_rnn:
						plot_id = itr // num_batches // n_iters_to_viz
						if itr == 2:
							viz_.draw_all_data(train_dict, model,
							plot_name=file_name + "_" + str(experimentID) + "_000_data.png",
							experimentID=experimentID, save=True)
						viz.draw_all_plots_one_dim(train_dict, model,
							plot_name = file_name + "_" + str(experimentID) + "_{:03d}".format(plot_id + 5000) + ".png",
						 	experimentID = experimentID, save=True)
						plt.pause(0.01)
	torch.save({
		'args': args,
