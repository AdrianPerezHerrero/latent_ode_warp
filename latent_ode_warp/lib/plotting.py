...
class Visualizations():
	def __init__(self, device, type='latent'):
		if type == 'latent':
			self.init_visualization()
		elif type == 'data':
			self.init_data_visualization()
	...
	def init_data_visualization(self):
		self.fig = plt.figure(figsize=(12, 7), facecolor='white')

		#Changed to warp
		self.ax_traj = []
		for i in range(1, 6):
			self.ax_traj.append(self.fig.add_subplot(5,1,i, frameon=False))
	...
	def draw_all_data(self, data_dict, model,
		plot_name = "", save = False, experimentID = 0.):

		data = data_dict["data_to_predict"]
		time_steps = data_dict["tp_to_predict"]
		# time_steps = data_dict["observed_tp"]
		mask = data_dict["mask_predicted_data"]
		...
		dim_to_show = 0
		n_traj_to_show = min(5, observed_data.shape[0])
		...
		for traj_id in range(0, n_traj_to_show):
		# Plot observations
